...
public class GridSectionAverageGapItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        if (parent.getLayoutManager() instanceof GridLayoutManager && parent.getAdapter() instanceof RecyclerViewAdapter) {
            ...
            int position = parent.getChildAdapterPosition(view) - mAdapter.getHeadCounts();

            if (position > mAdapter.getHeadCounts()) {
                //不处理header
                outRect.set(0, 0, 0, 0);
...
//            Log.w("GridAverageGapItem", "pos=" + position + ",vPos=" + visualPos + "," + outRect.toShortString());
    ...
    private void setUpWithAdapter(RecyclerViewAdapter adapter) {
        if (mAdapter != null) {
    ...
    private void markSections() {
        if (mAdapter != null) {
            RecyclerViewAdapter adapter = mAdapter;
            mSectionList.clear();
            Section section = new Section();
            for (int i = 0, size = adapter.getItemCount(); i < size; i++) {
                if (i > adapter.getHeadCounts()) {
                    //找到新Section起点
                    if (section != null && i != 0) {
                        //已经有待添加的section
                        section.endPos = i - 1;
