...
/**
 * @author caoyangfei
 * @ClassName VibrateBeepUtils
 * @date 2019/4/20
 * ------------- Description -------------
 * 播放“bee”的声音, 并且震动辅助类
 */
public final class VibrateBeepUtils implements Closeable {

    // 日志 TAG
    private static final String TAG = VibrateBeepUtils.class.getSimpleName();
    // Activity
     ...
     */
    public VibrateBeepUtils(final Activity activity) {
     ...
     */
    public VibrateBeepUtils(final Activity activity, @RawRes final int rawId) {
        this.mActivity = activity;
     ...
     * @param filePath 文件路径
     */
    public VibrateBeepUtils(final Activity activity, final String filePath) {
        this.mActivity = activity;
    ...
    private boolean shouldBeep() {
        try {
            ...
            LogUtils.tag(TAG).e("shouldBeep", e);
        }
        return true;
    }
     ...
     * @return {@link VibrateBeepUtils}
     */
    public VibrateBeepUtils setVibrate(final boolean vibrate) {
     ...
     * @return {@link VibrateBeepUtils}
     */
    public VibrateBeepUtils setVibrate(final boolean vibrate, final long vibrateDuration) {
     ...
     * @return {@link VibrateBeepUtils}
     */
    public VibrateBeepUtils setMediaPlayer(final MediaPlayer mediaPlayer) {
    ...
    public static MediaPlayer buildMediaPlayer(final Context context, @RawRes final int rawId, final float beepVolume) {
        ...
        mediaPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
            ...
            public void onCompletion(MediaPlayer mp) {
                LogUtils.tag(TAG).d("buildMediaPlayer - onCompletion");
            }
        });
        mediaPlayer.setOnErrorListener(new MediaPlayer.OnErrorListener() {
            @Override
            public synchronized boolean onError(MediaPlayer mp, int what, int extra) {
                LogUtils.tag(TAG).d("buildMediaPlayer - onError => what: " + what + ", extra: " + extra);
                // 播放异常, 直接不处理
                return true;
            }
        });
        try {
            ...
            LogUtils.tag(TAG).e("buildMediaPlayer", e);
