...
import com.flyang.util.log.file.LogFileParam;
import com.flyang.util.log.parser.LocalParserManager;
...
import static com.flyang.util.log.config.LogLevel.TYPE_DEBUG;
import static com.flyang.util.log.config.LogLevel.TYPE_ERROR;
import static com.flyang.util.log.config.LogLevel.TYPE_INFO;
import static com.flyang.util.log.config.LogLevel.TYPE_VERBOSE;
import static com.flyang.util.log.config.LogLevel.TYPE_WARM;
import static com.flyang.util.log.config.LogLevel.TYPE_WTF;
import static com.flyang.util.log.export.ExportLog.DIVIDER_BOTTOM;
import static com.flyang.util.log.export.ExportLog.DIVIDER_CENTER;
import static com.flyang.util.log.export.ExportLog.DIVIDER_NORMAL;
import static com.flyang.util.log.export.ExportLog.DIVIDER_TOP;
...
public class Logger implements Printer {
    ...
    private void logString(@LogLevelType int type, String msg, String tag, boolean isPart, Object... args) {
        ...
        // 超过长度分条打印
        if (msg.length() > ObjectUtils.LINE_MAX) {
            if (mLogConfig.isShowBorder()) {
                printLog(type, tag + CHARACTER_TABLE[0], ExportLog.printDividingLine(DIVIDER_TOP));
                printLog(type, tag + CHARACTER_TABLE[1], ExportLog.printDividingLine(DIVIDER_NORMAL) + getTopStackInfo());
                printLog(type, tag + CHARACTER_TABLE[2], ExportLog.printDividingLine(DIVIDER_CENTER));
            }
            for (String subMsg : ObjectUtils.largeStringToList(msg)) {
                logString(type, subMsg, tag, true, args);
            }
            if (mLogConfig.isShowBorder()) {
                printLog(type, tag + CHARACTER_TABLE[4], ExportLog.printDividingLine(DIVIDER_BOTTOM));
            }
            return;
        }
        // 分条打印日志
        if (mLogConfig.isShowBorder()) {
            if (isPart) {
                for (String sub : msg.split(Parser.LINE_SEPARATOR)) {
                    printLog(type, tag + CHARACTER_TABLE[3], ExportLog.printDividingLine(DIVIDER_NORMAL) + sub);
                }
            } else {
                printLog(type, tag + CHARACTER_TABLE[0], ExportLog.printDividingLine(DIVIDER_TOP));
                printLog(type, tag + CHARACTER_TABLE[1], ExportLog.printDividingLine(DIVIDER_NORMAL) + getTopStackInfo());
    ...
    private void logObject(@LogLevelType int type, Object object) {
        logString(type, ObjectUtils.objectToString(object));
    }
